...
import { BrandMark } from '../src/index.js';
...
describe('@kolektiv/brand-react BrandMark', () => {
  it('exposes per-part style hooks and colors', () => {
    const html = renderToStaticMarkup(
      <BrandMark colors={{ iconMark: '#d8a5f0' }} color="#c9a9ee" />,
    );
  ...
  it('omits hidden parts', () => {
    const html = renderToStaticMarkup(<BrandMark parts={{ iconMark: false }} />);
  ...
  it('renders exactly the shared inner markup', () => {
    ...
    const html = renderToStaticMarkup(<BrandMark {...options} />);
    expect(innerOf(html)).toBe(renderMarkChildren(options));
  });

  it('sets role/aria-label and a title element when titled', () => {
    const html = renderToStaticMarkup(<BrandMark title="Kolektiv" />);
